...
# Helper function to show available scripts in current directory
show_package_scripts() {
    ...
    if [[ -n "$npm_scripts" ]]; then
        echo "📜 npm/yarn/bun scripts (package.json):"
        echo "$npm_scripts" | while IFS=$'\t' read -r name cmd; do
            printf "  %-20s %s\n" "$name" "$cmd"
        done
        echo ""
    fi
    ...
    # Show deno tasks
    local deno_tasks="$(_get_deno_tasks)"
    if [[ -n "$deno_tasks" ]]; then
        echo "🦕 deno tasks:"
        echo "$deno_tasks" | while IFS=$'\t' read -r name cmd; do
            printf "  %-20s %s\n" "$name" "$cmd"
        done
        echo ""
    fi
    
    if [[ -z "$npm_scripts" && -z "$deno_tasks" ]]; then
        echo "  No package.json or deno.json configuration found"
    fi
}

echo "🚀 fzf-package-completions plugin loaded!"
echo "📋 Use tab completion with yarn, npm, bun, and deno commands"
echo "📋 Run 'show_package_scripts' to see available scripts"
echo "📋 Behavior:"
echo "   • yarn/bun: Native commands + package.json scripts"
echo "   • yarn run/bun run: Only package.json scripts"
echo "   • npm: Only native commands"
echo "   • npm run: Only package.json scripts"
echo "   • deno: Native commands + deno tasks"
echo "   • deno run: Scripts + tasks + files"
echo "   • deno task: Only deno tasks"